...
const CANDIDATES: { base: string; encoded: boolean }[] = [
  ...
  { base: 'https://r.jina.ai/http://demonslayerapi.com/api/v1/characters', encoded: false }, 
  { base: 'https://r.jina.ai/http://demonslayerapi.com/api/v1/characters%3F', encoded: true }, 
...
async function fetchText(url: string, timeoutMs = 15000): Promise<string> {
  ...
  if (hasAbort) {
    ...
    try {
      const res = await fetch(url, { signal: ctrl.signal, headers: COMMON_HEADERS });
...
function toList(raw: any): Character[] {
  ...
  if (raw && Array.isArray(raw.characters)) return raw.characters as Character[];
  return [];
}
